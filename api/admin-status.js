...
const statusMessages = {
  "in behandeling": {
    ...
    body: (name, ref, readyBy) => {
      const dateStr = readyBy ? ` We verwachten je kledingstuk klaar te hebben op <strong>${readyBy}</strong>.` : "";
      return `<p style="font-size:14px;color:#20201f;margin:0 0 16px;line-height:1.7;">Hoi ${name}, goed nieuws — je kledingstuk is aangekomen en we zijn begonnen met de reparatie.${dateStr}</p><p style="font-size:13px;color:#918b84;margin:0;line-height:1.7;">Heb je vragen? WhatsApp <a href="https://wa.me/31616120895" style="color:#b85c38;text-decoration:none;">+31 6 16120895</a> met referentie ${ref}.</p>`;
    }
  },
  "klaar": {
    subject: (ref) => `KnitFix · je reparatie is klaar (${ref})`,
    body: (name, ref) => `<p style="font-size:14px;color:#20201f;margin:0 0 16px;line-height:1.7;">Hoi ${name}, je reparatie is klaar! Je ontvangt binnenkort een factuur voor het restbedrag. Zodra betaald sturen we je kledingstuk terug.</p><p style="font-size:13px;color:#918b84;margin:0;line-height:1.7;">Vragen? <a href="https://wa.me/31616120895" style="color:#b85c38;text-decoration:none;">+31 6 16120895</a> · ref: ${ref}</p>`
  },
  "verzonden": {
    subject: (ref) => `KnitFix · je pakket is onderweg (${ref})`,
    body: (name, ref) => `<p style="font-size:14px;color:#20201f;margin:0 0 16px;line-height:1.7;">Hoi ${name}, je kledingstuk is verstuurd en is binnenkort bij je thuis. Bedankt voor je vertrouwen in KnitFix!</p><p style="font-size:13px;color:#918b84;margin:0;line-height:1.7;">Referentie: ${ref} · <a href="https://wa.me/31616120895" style="color:#b85c38;text-decoration:none;">+31 6 16120895</a></p>`
  }
};
...
<div style="max-width:520px;margin:0 auto;background:#fff;border:1px solid #ded6cf;">
  <div style="padding:36px 40px 24px;border-bottom:1px solid #ded6cf;">
    ...
    <h1 style="font-size:18px;font-weight:400;color:#20201f;margin:0;text-transform:lowercase;">${subject.split("·")[1]?.trim() || "update"}</h1>
  </div>
  <div style="padding:32px 40px;">${bodyHtml}</div>
  <div style="padding:20px 40px;border-top:1px solid #ded6cf;">
    <p style="font-size:10px;color:#918b84;margin:0;">knitfix · amsterdam · <a href="https://knitfix.nl" style="color:#918b84;">knitfix.nl</a> · kvk 42013270</p>
  </div>
</div>
</body></html>`;
}
  ...
  if (template && email) {
    try {
      ...
      await resend.emails.send({
        ...
        to: email,
        subject,
        html: buildEmail(subject, template.body(name, ref, ready_by)),
      });
    } catch (err) {
